...
\hypersetup{
  pdftitle={AMOS: An Internally Deployed Analyst System},
...
\begin{center}
  {\fontsize{20}{23}\selectfont\bfseries\textcolor{amosblue}{AMOS: An Internally Deployed\\Analyst System}}
\end{center}
...
\begin{enumerate}
  ...
  \item \textbf{Analyst and control runtime:} a customer-local model proposes plans and interpretations; a governed execution environment controls read-only SQL, deterministic statistics and chart tools, verification, review, scheduling, and recovery.
  \item \textbf{Artifact product:} deterministic compilers produce graphs, reports, editable presentation slides, dashboards, and spreadsheets with claim-level evidence.
\end{enumerate}

The required end-to-end flow is: employee or scheduled request $\rightarrow$ local analyst agent (initially Gemma 4) $\rightarrow$ \amos control layer $\rightarrow$ company data and tool connectors plus bounded SQL/statistics/chart workers $\rightarrow$ verification and evidence $\rightarrow$ report and slide plan $\rightarrow$ deterministic artifact compiler $\rightarrow$ charts, PPTX, PDF, dashboard, and spreadsheet.
